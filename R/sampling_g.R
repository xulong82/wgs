...
rm(list = ls())
load("/data/xwang/glmm/sampling_Apoe.rdt")

waic = lapply(y, function(x) x$waic) %>% unlist
summary = lapply(y, function(x) x$summary)
p = lapply(summary, function(x) sapply(x, function(x1) x1["p", ]))
p = t(do.call(cbind, p))

load("/data/xwang/glmm/glmm/chr19.rdt")
fit = fit[rownames(p), ]

save(waic, summary, p, fit, file = "~/Dropbox/GitHub/glmm/Apoe/sampling.rdt")
load(file = "~/Dropbox/GitHub/glmm/Apoe/sampling.rdt")

plot(fit[, "p"], p[, "mean"])
plot(fit[, "se"], p[, "sd"])

p_fit = pnorm(abs(fit[, "p"]), sd = fit[, "se"], lower.tail = F) * 2
p_sam = pnorm(abs(p[, "mean"]), sd = p[, "sd"], lower.tail = F) * 2

plot(-log10(p_fit), -log10(p_sam))

pos = gsub("^19-", "", rownames(fit)) %>% as.numeric
plot(pos, -log10(p_fit))
plot(pos, -log10(p_sam))
