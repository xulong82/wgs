...
model <- stan_model("~/Dropbox/GitHub/glmm/Stan/noK.stan")
dat = list(N = 576, K = 4, D = 2, cov = cov, g = g0, Ad = Ad)

# fit <- optimizing(model, data = dat)
# init = list(p = 0, beta = fit$par[paste0("beta[", 1:2, "]")], c = fit$par[paste0("c[", 1:3, "]")])
# save(init, file = "~/Dropbox/GitHub/glmm/data/init_noK.rdt")
load("~/Dropbox/GitHub/glmm/data/init_noK.rdt")
...
save(fit, file = paste0("/data/xwang/glmm/noK/", chr, ".rdt"))
